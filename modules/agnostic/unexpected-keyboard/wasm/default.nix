...
let
  modulePath = <collective/collective-public/modules/agnostic/unexpected-keyboard>;

  mkConfiguration = mkKeyboard: {config, ...}: {
    services.unexpected-keyboard = {
      enable = true;
      includeDefaultKeyboards = false;
      keyboards = [(mkKeyboard config)];
    };
  };

  mkLayouts = mkKeyboard:
    with typed.tests.evalTestableModule modulePath (mkConfiguration mkKeyboard);
    forAttrs services.unexpected-keyboard.layouts (_: layout: layout.xmlSource);

in
  mkLayouts (config:
    with config.services.unexpected-keyboard.lib;
