pkgs: rec {
  python312 = {
    packages = {
      handheld-daemon = rec {
        adjustor = pkgs.callPackage ./pythonPackages/handheld-daemon/adjustor.nix {  };
        hhd = pkgs.callPackage ./pythonPackages/handheld-daemon/hhd.nix { inherit adjustor; };
      };
    };
  };
  handheld-daemon = pkgs.pythonPackages.toPythonApplication python312.packages.handheld-daemon.hhd;
  handheld-daemon-adjustor = pkgs.pythonPackages.toPythonApplication python312.packages.handheld-daemon.adjustor;
  handheld-daemon-ui = pkgs.callPackage ./handheld-daemon-ui.nix { };
}
