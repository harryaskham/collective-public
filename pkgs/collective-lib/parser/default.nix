      ...
      evalNodeWithScope = scope: node:
        if !(isNode node) then node
          ...
          let
            ...
            # with attributes are fallbacks - existing lexical scope bindings shadow them
          ...
          in
            # In Nix, if condition is a string, use it as error message and fail
            if builtins.isString condResult then
              throw condResult
            else if condResult then 
              evalNodeWithScope scope node.body
            else 
